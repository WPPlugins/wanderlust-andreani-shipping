...
class WC_Shipping_Andreani extends WC_Shipping_Method {
	...
	*/
	...
	public function calculate_shipping( $package = array() ) {
		...
		$packages_to_quote_count = sizeof( $andreani_requests );
		...
		foreach ($andreani_packages as $key) {
			$andreani_package = $key['GroupPackageCount'];
	 		$andreani_weight = $key['Weight']['Value'];
			$andreani_lenth = $key['Dimensions']['Length'] / 100;
			$andreani_width = $key['Dimensions']['Width'] / 100;		
			$andreani_height = $key['Dimensions']['Height'] / 100;	
		...
		$seguro = round($package['contents_cost']);
 		...
 		if($mercado_pago =='1'){
			add_filter( 'woocommerce_cart_shipping_method_full_label', 'remove_landreanil_pickup_free_label', 10, 2 );
			function remove_landreanil_pickup_free_label($full_label, $method){
				$full_label = str_replace("(Gratis)","",$full_label);
			return $full_label;
			}
			add_filter( 'woocommerce_cart_shipping_method_full_label', 'remove_landreanil_pickup_free_label_en', 10, 2 );
			function remove_landreanil_pickup_free_label_en($full_label, $method){
			...
			if($services['enabled'] == 1){
			 ...
			 curl_setopt_array($ch,	
														array(	
															...
															CURLOPT_URL => $wp_session['url_andreani'],
															CURLOPT_RETURNTRANSFER => TRUE,
															CURLOPT_FOLLOWLOCATION	=> TRUE
														)
													);

			 	$andreani_response = curl_exec ($ch);		
        $andreani_response = json_decode($andreani_response);				
				...
				$porcentaje = $andreani_response->results * $ajuste / 100;
				
				$precio = $andreani_response->results + $porcentaje;
				$precio_base = $andreani_response->results + $porcentaje;
				...
				if($mercado_pago =='1'){
					...
					$precio = '0';
					$rate = array(
						'id' => sprintf("%s-%s", $titulo, $services['service_name'] . '-' . $services['woocommerce_andreani_wanderlust_modalidad'] . 'api_nrocuenta' . $this->api_nrocuenta . 'operativa' . $services['operativa'] ),
